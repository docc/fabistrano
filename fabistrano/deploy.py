...
@with_defaults
def collectstatic():
    run("source %(venv_path)s/bin/activate;cd %(current_release)s;python %(manage_path)s/manage.py collectstatic --noinput" % { 'shared_path':env.shared_path, 'current_release':env.current_release, 'manage_path':env.manage_path })
