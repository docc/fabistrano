...
@with_defaults
def collectstatic():
    run("source %(venv_path)s/bin/activate;cd %(current_release)s;python %(manage_path)smanage.py collectstatic --noinput --settings=%(settings)s" % { 'venv_path':env.venv_path, 'current_release':env.current_release, 'settings':env.settings, 'manage_path':env.manage_path })

@with_defaults
def migrate():
    run("source %(venv_path)s/bin/activate;cd %(current_release)s;python %(manage_path)smanage.py migrate --noinput --settings=%(settings)s" % { 'venv_path':env.venv_path, 'current_release':env.current_release, 'settings':env.settings, 'manage_path':env.manage_path })
